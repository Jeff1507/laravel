...
@section('content')
    <div class="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        <div class="flex flex-col gap-2 w-full items-center justify-center">
            ...
            <h2 class="text-2xl font-bold text-gray-700 mb-4">Editar Produto</h2>
        </div>
        <form action="{{ route('produtos.update', $produto->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nome" id="nome" class="w-full p-2 border rounded" pattern="^[^ ].+[^ ]$" value="{{ old('nome', $produto->nome) }}" required>
                </div>
                <div>
                    <label for="imagem" class="block font-semibold text-gray-600">Imagem</label>
                    <img id="imagemPreview" src="{{ asset('img/produtos/' . $produto->imagem) }}" alt="Imagem do Produto" class="max-w-full max-h-40 mx-auto mt-2">
                ...
                <div>
                    ...
                    <input type="number" name="estoque" id="estoque" required min="0" class="w-full p-2 border rounded" value="{{ old('estoque', $produto->estoque) }}">
                </div>

                <div>
                    <label for="valor_unitario" class="block font-semibold text-gray-600">Valor Unitário</label>
                    <input type="text" name="valor_unitario" id="valor_unitario" required class="w-full p-2 border rounded" value="{{ old('valor_unitario', $produto->valor_unitario) }}">
                </div>

                <div class="col-span-full">
                    <label for="descricao" class="block font-semibold text-gray-600">Descrição</label>
                    <textarea name="descricao" id="descricao" class="w-full p-2 border rounded">{{ old('descricao', $produto->descricao) }}</textarea>
                </div>
            ...
            <div class="mt-4 flex items-center justify-between">
                ...
                <input type="submit" value="Editar Produto" class="bg-blue-600 cursor-pointer text-white px-4 py-2 rounded-lg hover:bg-blue-800">
            </div>
        </form>
    </div>
    <script>
